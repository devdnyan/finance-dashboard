...
export default function LChart() {
    ...
    return (
        <div className="flex h-[21rem] w-full flex-col rounded-2xl border border-indigo-100 bg-gradient-to-b from-white to-indigo-50/40 p-4 shadow-[0_8px_20px_-16px_rgba(79,70,229,0.5)] sm:h-[22rem] sm:p-5 xl:h-full xl:min-h-0 dark:border-indigo-900/50 dark:from-slate-900 dark:to-indigo-950/35">
            <h2 className="mb-3 text-sm font-bold uppercase tracking-[0.08em] text-indigo-700 dark:text-indigo-300">Cash Flow Trend</h2>

            <div className="min-h-0 flex-1">
                <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={data} margin={{ top: 5, right: 10, left: -30, bottom: 5 }}>

                    <CartesianGrid strokeDasharray="3 3" vertical={false} stroke={isDark ? '#334155' : '#e0e7ff'} />
                    ...
                    <ReferenceLine y={0} stroke={isDark ? '#a5b4fc' : '#6366f1'} strokeDasharray="3 3" />

                    <Line 
                        type="monotone" 
                        dataKey="amount" 
                        stroke={isDark ? '#a5b4fc' : '#4338ca'} 
                        strokeWidth={3}
                        dot={{ r: 4, fill: isDark ? '#a5b4fc' : '#4338ca', strokeWidth: 2, stroke: isDark ? '#0f172a' : '#eef2ff' }}
